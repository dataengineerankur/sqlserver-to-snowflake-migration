"""
Snowpark Python stored procedures replacing the two SQL Server XML procs:

  dbo.usp_Stress_XmlOrderDocument(@OrderId)  → SP_XML_ORDER_DOCUMENT(order_id)
  dbo.usp_Inv_StockXml(@WhId)                → SP_INV_STOCK_XML(wh_id)

Snowflake has no native XML type, so these produce XML strings via Python's
standard library. Deploy with:

  snowsql -q "$(cat 02_xml_snowpark.py | python3 -c 'import sys; print(sys.stdin.read())')"

Or let Terraform apply the CREATE PROCEDURE statements below by referencing
this file content in the procedure_body locals in udfs.tf.
"""
from __future__ import annotations


SP_XML_ORDER_DOCUMENT_SQL = """\
CREATE OR REPLACE PROCEDURE BRONZE.SP_XML_ORDER_DOCUMENT(P_ORDER_ID FLOAT)
RETURNS VARCHAR
LANGUAGE PYTHON
...
def run(session, p_order_id: float) -> str:
    ...
    for line in line_rows:
        ...
        ET.SubElement(line_el, 'ProductName').text = line['PRODUCT_NAME']
...
SP_INV_STOCK_XML_SQL = """\
CREATE OR REPLACE PROCEDURE BRONZE.SP_INV_STOCK_XML(P_WH_ID FLOAT)
RETURNS VARCHAR
LANGUAGE PYTHON
RUNTIME_VERSION = '3.11'
PACKAGES = ('snowflake-snowpark-python')
...
if __name__ == '__main__':
    print(SP_XML_ORDER_DOCUMENT_SQL)
    print(SP_INV_STOCK_XML_SQL)
